...
- After merge, verify the resulting post-merge CI/validation result with
  the `github-webhook-watch` skill when a workflow is expected. Deployment is
  not expected in this repo because deploy/ops workflows are retired.
- Do not yield to the user until the PR is merged and post-merge verification is
  either complete or explicitly blocked; keep the watcher loop running unless
  blocked.
...
10. Immediately after the merge, run the **Post-merge CI/validation verification**
    flow below. Do not report the PR as fully landed until this verification has
    a clear result (success, failure, no CI/deployment expected, or blocked).
...
## Post-merge CI/validation verification

After `gh pr merge` succeeds, verify the main-branch CI/validation outcome before
posting a final success update. Deployment is not expected in this repo unless a
future explicit non-Actions deploy mechanism says otherwise.

1. Capture the merge result and current main state:
   ```
   gh pr view "$pr_number" --json url,state,mergedAt,mergeCommit
   git fetch origin main
   git rev-parse origin/main
   ```
2. Determine whether post-merge CI is expected.
   - GitHub Actions deploy/ops workflows are retired in this repo.
   - Only secret-free validation CI may run after #124.
   - If no workflow run is expected, report `no CI/deployment expected` with the reason.
3. If a validation workflow is expected, use the `github-webhook-watch` skill in the
   same Discord thread/channel to watch it. Include repo, PR URL, merge commit SHA,
   expected workflow names, and desired terminal states.
4. The final user-visible landing report must include one of:
   - `✅ validation succeeded` with the workflow/run URL
   - `❌ validation failed/cancelled` with the workflow/run URL and next log to inspect
   - `ℹ️ no CI/deployment expected` with the reason and evidence checked
   - `⚠️ verification blocked` with the exact missing permission or unavailable signal
5. Do not claim the PR is fully done before this post-merge verification result
   is known.
